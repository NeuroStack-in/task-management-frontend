...
// ── In-page section anchors (drive the sticky sub-nav) ────────────────────────
const SECTIONS = [
  { id: "idle", label: "Idle detection" },
  { id: "screenshots", label: "Screenshots" },
  { id: "productivity", label: "Productivity" },
  { id: "work-hours", label: "Work hours" },
  { id: "alerts", label: "Alert thresholds" },
  { id: "silent", label: "Silent monitoring" },
] as const
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:gap-8">
        {/* ── Sticky sub-nav (anchors into the long card stack) ── */}
        <nav
          aria-label="Monitoring sections"
          className="lg:sticky lg:top-20 lg:w-48 lg:shrink-0 lg:self-start"
        >
          <ul className="flex gap-1 overflow-x-auto pb-1 lg:flex-col lg:gap-0.5 lg:overflow-visible lg:pb-0">
            {SECTIONS.map((s) => (
              <li key={s.id} className="shrink-0">
                <a
                  href={`#${s.id}`}
                  className="block whitespace-nowrap rounded-md px-3 py-2 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
                >
                  {s.label}
                </a>
              </li>
